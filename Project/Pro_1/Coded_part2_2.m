function [BER_soft_g1,BER_soft_g2,BER_soft_g3,trellis] = Coded_part2_2(N,maxNumErrs,maxNum,EbN0)
...
for i = 1:length(EbN0) % use parfor ('help parfor') to parallelize  
  ...
  while((totErr_soft_g1 < maxNumErrs) && (num < maxNum)) 
  % ===================================================================== %
...
%   C_encoded= gen_4(bits); % 4th encoder
  ...
  %% [MOD] Symbol Mapper - Mode is based on the flags set in the beginning.
    ...
    [symb_coded,const,trellis] = symbol_mapper(bits,modulation);
%     symb_coded
%     trellis
...
% v_bits = viterbi_softDecoding(y,const);
    ...
    bits_decoded_g2 = vitdec(code2,trellis(2,:),tblen,'term','soft',3);
    bits_decoded_g2 = bits_decoded_g2(1:end-4); % discard four end bits
    err_soft_g2  = bits(1:end)-bits_decoded_g2; % compare to find errors
    ...
    bits_decoded_g3 = vitdec(code3,trellis(3,:),tblen,'term','soft',3);
    bits_decoded_g3 = bits_decoded_g3(1:end-4); % discard four end bits
    err_soft_g3  = bits(1:end)-bits_decoded_g3; % compare to find errors
  ...
  BitErrs_soft_g2 = (sum(abs(err_soft_g2).^2)); % count the bit errors and evaluate the bit error rate
  totErr_soft_g2 = totErr_soft_g2 + BitErrs_soft_g2;
  
  
  BitErrs_soft_g3 = (sum(abs(err_soft_g3).^2)); % count the bit errors and evaluate the bit error rate
  totErr_soft_g3 = totErr_soft_g3 + BitErrs_soft_g3;
...
%       num2str(totErr/num, '%10.1e') '. +++']);
  ...
  BER_soft_g2(i) = totErr_soft_g2/num;
  BER_soft_g3(i) = totErr_soft_g3/num;
end
BER_soft_g1 = BER_soft_g1;
BER_soft_g2 = BER_soft_g2;
BER_soft_g3 = BER_soft_g3;
