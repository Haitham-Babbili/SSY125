...
%     case 1
%     %% convolution encoder G = (1 + D2 + D3 + D4, 1 + D2 + D3)
%     inital_state=[0 0 0 0];   %initial state
%     updated_state=[0 0 0 0];  % updated state
%     
%     for i=1:length(pack)
% j=1;
% tem1=xor(inital_state(j+1),inital_state(j+2));
% tem2=xor(tem1,inital_state(j+3));
% %tem3=xor(tem2,inital_state(j+3));
% o1=xor(pack(i),tem2); % o1=xor(u,D^2,D^3,D^4)
% %tem=xor(inital_state(j),inital_state(j+1));
% o2=xor(pack(i),tem1); % o2=xor(u,D,D^2)
% pack_codewrd(i,:)=[o1 o2]; %bits into encoded codewords
% 
%  % updating state by shifting input bits into initial state
% bit=inital_state(1,1);
...
%     end
%     %case 2
%      %% convolution encoder G = (1 + D3 + D4, 1 + D + D3 + D4)
%     inital_state=[0 0 0 0];   %initial state
%     updated_state=[0 0 0 0];  % updated state
%     
%     for i=1:length(pack)
% j=1;
% tem_1=xor(inital_state(j+2),inital_state(j+3));
% %temr2=xor(tem1,inital_state(j+3));
% %tem3=xor(tem2,inital_state(j+3));
% o1=xor(pack(i),temr1); % o1=xor(u,D^2,D^3,D^4)
% tem_2=xor(inital_state(j),inital_state(j+2));
% tem_3=xor(tem_2,inital_state(j+3));
% o2=xor(pack(i),tem_3); % o2=xor(u,D,D^2)
% pack_codewrd(i,:)=[o1 o2]; %bits into encoded codewords
% 
%  % updating state by shifting input bits into initial state
% bit=inital_state(1,1);
